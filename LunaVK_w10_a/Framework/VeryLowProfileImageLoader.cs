...
namespace LunaVK.Framework
{
    public static class VeryLowProfileImageLoader
    {
        ...
        private static readonly Task _task = new Task(VeryLowProfileImageLoader.WorkerThreadProc);
        ...
        public static readonly DependencyProperty UriSourceProperty = DependencyProperty.RegisterAttached("UriSource", typeof(Uri), typeof(VeryLowProfileImageLoader), new PropertyMetadata(null,VeryLowProfileImageLoader.OnUriSourceChanged));
...
//        private const int WorkItemQuantum = 5;
        ...
        {
            VeryLowProfileImageLoader._task.Start();//VeryLowProfileImageLoader._thread.Start();
        ...
        public static void SetUriSource(Image obj, Uri value)
        {
            if (obj == null)
                throw new ArgumentNullException("obj");
            obj.SetValue(VeryLowProfileImageLoader.UriSourceProperty, value);
        }
        ...
        {
            obj.SetValue(VeryLowProfileImageLoader.UseBackgroundCreationProperty, value);
        }
                ...
                if (num3 >= VeryLowProfileImageLoader.MaxSimultaneousLoads || list.Count <= 0)
                {
                    ...
                    while (0 < queue.Count && num4 < 5)
                    {
                        ...
                        try
                        {
                            ...
                            Stream temp = responseState.WebRequest.EndGetRequestStream(asyncResult);
                            //Stream temp = webResponse.GetResponseStream();
                            pendingCompletions.Enqueue(new VeryLowProfileImageLoader.PendingCompletion(responseState.Image, responseState.Uri, temp, responseState.Timestamp, responseState.RequestId));
                        }
                    ...
                    if (0 < pendingCompletions.Count)
                    {
                        ...
                        //Deployment.Current.Dispatcher.BeginInvoke(delegate
                        {
                            ...
                            while (0 < pendingCompletions.Count)
                            {
                                ...
                                VeryLowProfileImageLoader.AddToCache(pendingCompletion.Uri, pendingCompletion.PendingStream);
                                if (VeryLowProfileImageLoader.GetUriSource(pendingCompletion.Image) == pendingCompletion.Uri)
                                {
                                    ...
                                    try
                                    {
                                        Stream temp = pendingCompletion.PendingStream;
                                        bitmapImage.SetSource(temp.AsRandomAccessStream());//mod
                                    }
                                    catch (Exception e2)
                                    {
//                                        Logger.Instance.Error("Error of reading image", e2);
                                    ...
                                    double totalMilliseconds = (DateTime.Now - pendingCompletion.Timestamp).TotalMilliseconds;
                                ...
                                pendingCompletion.PendingStream.Dispose();
                            }
                            try
                            {
...
//                httpWebRequest2.AllowReadStreamBuffering = true;
        ...
        public static BitmapImage CreateSizedBitmap(FrameworkElement container, Uri uri, bool backgroundCreation = true)
        {
            ...
            if (container.Height > 0.0 && container.Width > 0.0)
            {
                ...
                else
                {
                    ...
                    bitmapImage1.DecodePixelWidth = (int)fill.Width;
                    // ISSUE: explicit reference operation
                    bitmapImage1.DecodePixelHeight = (int)fill.Height;
                }
            }
            else
        ...
        private static void AddToCache(Uri uri, Stream stream)
        {
            ...
            VeryLowProfileImageLoader._lruCache.Add(uri, StreamUtils.ReadFully(stream), true);
            if (stream.CanSeek)//mod
                stream.Position = 0;
            if (UriExtensions.ParseQueryString(uri).ContainsKey(VeryLowProfileImageLoader.REQUIRE_CACHING_KEY))
                ImageCache.Current.TrySetImageForUri(uri.OriginalString, StreamUtils.ReadFully(stream));
            if (stream.CanSeek)//mod
                stream.Position = 0;
        }
        ...
        private static void EnsureLRUCache()
        {
            if (VeryLowProfileImageLoader._lruCache != null)
                return;
            VeryLowProfileImageLoader._lruCache = new LRUCache<Uri, Stream>(VeryLowProfileImageLoader.LRU_CACHE_CAPACITY, (s => (int)s.Length));
        }
...
//            Logger.Instance.Info(info);
        ...
        private static void OnUriSourceChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            else
            {
                ...
                //
                if (stream==null)
                    stream = ImageCache.Current.GetCachedImageStream(newValue.OriginalString).Result;
                //
                if (stream != null)
                {
                    ...
                    sizedBitmap.SetSource(stream.AsRandomAccessStream());//mod
        ...
        private static void DisposeImage(BitmapImage img)
        {
            ...
            try
            {
                using (MemoryStream memoryStream = new MemoryStream((byte[])new byte[1]))
                    img.SetSource(memoryStream.AsRandomAccessStream());//mod
            }
            catch
            {
            }
        ...
        private class PendingCompletion
        {
            ...
            public Stream PendingStream { get; private set; }
