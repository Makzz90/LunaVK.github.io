...
//                this._isThicknessPopupAnimating = false;
        ...
        */
        ...
        private async void Action_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            //
            string uriString = CacheManager.NewGuid("graffiti.png");//string.Format("/{0}", Guid.NewGuid());
            //IBuffer pixels = await bitmap.GetPixelsAsync();
            //IRandomAccessStream stream = new InMemoryRandomAccessStream();
            System.IO.Stream responseStream = await new LunaVK.Common.GraffitiEncoder(bitmap).Encode();
            Framework.ImageCache.Current.TrySetImageForUri(uriString, responseStream);


            this._isSaving = false;
            
            //
            this.SendAction?.Invoke(bitmap, ApplicationData.Current.LocalFolder.Path + "\\" + uriString);
        }
        ...
        private async Task<RenderTargetBitmap> CreateRenderBitmap()
        {
            ...
            int scaledWidth = (int)this.drawCanvas.ActualWidth * mult;
            int scaledHeight = (int)this.drawCanvas.ActualHeight * mult;
            

            await renderTargetBitmap.RenderAsync(this.drawCanvas, scaledWidth, scaledHeight);
        ...
        {
            UIElement canvs = sender as UIElement;
            var ttv = canvs.TransformToVisual(Window.Current.Content);
            Point screenCoords = ttv.TransformPoint(new Point(0, 0));
