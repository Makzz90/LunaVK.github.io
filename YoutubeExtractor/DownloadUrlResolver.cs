...
namespace YoutubeExtractor
{
    ...
    public static class DownloadUrlResolver
    {
        ...
        private const string SignatureQuery = "sig";//signature
        ...
        public static void GetDownloadUrls(string videoUrl, Action<IEnumerable<VideoInfo>> callback, bool decryptSignature = true)
        {
            if (videoUrl == null)
        ...
        */
        ...
        private static IEnumerable<ExtractionInfo> ExtractDownloadUrls(JObject json)
        {
            ...
            JToken formats = GetStreamMap(json);
        ...
        private static void ExtractInfo(List<ExtractionInfo> info, JToken items)
        {
            if (items != null)
            {
                foreach (var item in items)
                {
                    ...
                    if (item["cipher"] != null || item["signatureCipher"] != null)
                    {
                        string s = (item["cipher"]!=null? item["cipher"] : item["signatureCipher"]).ToString();
                        IDictionary<string, string> queries = HttpHelper.ParseQueryString(s);
        ...
        */
        ...
        /*
        private static string GetHtml5PlayerVersion(JObject json)//v1
        {
        ...
        private static string GetHtml5PlayerVersion(JObject json)//v2
        {
        ...
        */
        private static string GetHtml5PlayerVersion(JObject json)//v3
        {
            string js = json["assets"]["js"].ToString();
        ...
        */
        ...
        private static IEnumerable<VideoInfo> GetVideoInfos(IEnumerable<ExtractionInfo> extractionInfos, string videoTitle)
        {
            ...
            foreach (ExtractionInfo extractionInfo in extractionInfos)
            {
                ...
                {
                    info = new VideoInfo(formatCode) { DownloadUrl = extractionInfo.Uri.ToString() };
                }
