    ...
    public static class LottieCompositionFactory
    {
        ...
        public static LottieResult<LottieComposition> FromAssetSync(CanvasDevice device, string fileName)
        {
            try
            {
                ...
                IsolatedStorageFileStream storageFileStream = storeForApplication.OpenFile(fileName, FileMode.Open);
                //
                //
                return FromJsonInputStreamSync(storageFileStream, cacheKey);
            }
            catch (IOException e)
            {
