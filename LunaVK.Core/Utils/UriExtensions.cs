...
namespace LunaVK.Core.Utils
{
    ...
    {
        private static readonly Regex QueryStringRegex = new Regex("(?<name>[^&=?]+)=(?<value>[^&=]+)");//Было [\\?&](?<name>[^&=]+)=(?<value>[^&=]+)
        ...
        public static bool IsExternal(this string uriString)
        {
            ...
            if (!uriString.StartsWith("http://", StringComparison.OrdinalIgnoreCase))
                return uriString.StartsWith("https://", StringComparison.OrdinalIgnoreCase);
            return true;
        }
        */
    }
